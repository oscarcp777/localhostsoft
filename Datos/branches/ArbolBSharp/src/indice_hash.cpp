...
IndiceHash::IndiceHash( Esquema::puntero esquema, Clave::puntero clavePrimaria,
                            ...
                            int ptamanio_bloque, const std::string& nombre_indice) throw(): EstrategiaIndice(clavePrimaria) {
	// Establece el esquema del indice
	this->esquemaIndice = esquema->clonar();
    ...
    this->almacenamiento_tabla = new Archivo( 8, nombre_indice + ".th.idx" );
    ((Archivo::puntero)this->almacenamiento_tabla)->abrirModoTexto();
    /*Se definen los almacenamientos*/
    this->almacenamiento_bucket = new Archivo( ptamanio_bloque+4, nombre_indice + ".bh.idx" );
...
Registro::puntero IndiceHash::buscar_registro(Registro::puntero registro) throw()
{
    ...
    if ( encontrado ==1 )
    {
        ...
        while (actual_campo_clave != fin_campo_clave) {
            ...
            Campo* campo = registro->obtener_campo(*actual_campo_clave);

            switch(campo->obtener_tipo_campo())
            {
                case TIPO_CAMPO_CADENA: {
                                        ca = new CampoCadena( clave_aux );
