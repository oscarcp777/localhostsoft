...
Archivo::Archivo(unsigned int longitud, const std::string& nombre_archivo, TipoAlmacenamiento tipo) throw(): Almacenamiento(longitud, tipo), nombre_archivo(nombre_archivo)
{
}

Archivo::Archivo(unsigned int longitud, const std::string& nombre_archivo, Esquema::puntero esquema, std::list<Clave::puntero>& listaClaves, TipoAlmacenamiento tipo) throw(): Almacenamiento(longitud, tipo)
{
...
Archivo::Archivo(unsigned int longitud, const std::string& nombre_archivo, Esquema::puntero esquema, EsquemaBloque::puntero esquemaBloque, std::list<Clave::puntero>& listaClaves, TipoAlmacenamiento tipo) throw(): Almacenamiento(longitud, tipo)
{
    //seteo nombres de archivo y de archivoHeader
    this->nombre_archivo = nombre_archivo;
    this->nombre_archivo_header = nombre_archivo;
...
int Archivo::LeerEspacioBloque( int posicion )  throw()
{
    ...
    char linea[1024*1024];
    char *a, *b;
    char cantidadBloques[12];
    char espacioOcupado[12];

    this->archivoEsquemaBloque.read(linea, 1024*1024);
...
void Archivo::leerHeaderModoTexto() throw()
{
	if (this->archivoHeader.is_open()) {
		...
		unsigned int cantidad_campos_esquema;
...
bool Archivo::escribirTexto(const char* bytes, unsigned int posicion) throw()
{
	unsigned int offset = posicion * this->obtener_longitud_en_bytes();
	this->archivo.seekp(offset, std::ios_base::beg);
	this->archivo << bytes;
...
bool Archivo::leer(char* bytes, unsigned int posicion) throw() {
	...
	int largo = this->obtener_longitud_en_bytes();
...
int Archivo::GetEspacioLibre()
{
    int cantidad_elementos = this->obtener_cantidad_registros();
	int contador = 0;
	bool encontrado = false;
	char* bytes = new char[this->obtener_longitud_en_bytes()];

	while (contador < cantidad_elementos && !encontrado)
	{
		this->leer(bytes, contador);
		std::string linea;
		linea.append(bytes);
		if(linea.length() == 0)
			encontrado = true;
